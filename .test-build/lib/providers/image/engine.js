...
exports.generateKeyframe = generateKeyframe;
const fal_1 = require("@/lib/fal");
const replicate_1 = require("@/lib/replicate");
function aspectToFalSize(ratio) {
...
function aspectToReplicateSize(ratio) {
    if (ratio === '9:16')
...
async function generateKeyframe(input) {
    const attempts = [];
    const aspect = input.width && input.height ? guessAspect(input.width, input.height) : undefined;
    if (isFalConfigured()) {
        try {
            const r = await (0, fal_1.generateFalImage)(input.prompt, aspectToFalSize(aspect));
    ...
    if (isReplicateConfigured()) {
        try {
            ...
            const r = await (0, replicate_1.generateReplicateFluxDev)(input.prompt, dims.width, dims.height);
    ...
    if (isReplicateConfigured()) {
        try {
            const dims = aspectToReplicateSize(aspect);
            const r = await (0, replicate_1.generateReplicateImage)(input.prompt, dims.width, dims.height);
