...
const AdCard = ({ ad, index, brand, productImage }: { ad: any, index: number, brand: any, productImage: string }) => {
    ...
    useEffect(() => {
        ...
        const newSrc = ad.generated_image_url || productImage || FALLBACK_IMAGE;
        console.log(`Ad #${index} Initial Source:`, newSrc);
        setImgSrc(newSrc);
        setHasError(false);
    }, [ad, productImage]);
    ...
    const handleImageError = (e: any) => {
        ...
        // 1. Try Product Image if Gen Image fails
        if (imgSrc === ad.generated_image_url && productImage) {
            console.log(`Ad #${index} Falling back to Product Image`);
            setImgSrc(productImage);
        }
        // 2. Try Fallback if Product Image fails or wasn't available
        else if (imgSrc !== FALLBACK_IMAGE) {
            console.log(`Ad #${index} Falling back to Placeholder`);
            setImgSrc(FALLBACK_IMAGE);
        }
        // 3. If Fallback fails (rare), show error overlay
        else {
            console.error(`Ad #${index} All image sources failed.`);
            setHasError(true);
        }
    };
...
export default function Dashboard() {
    ...
    const [count, setCount] = useState(3); // New state for bulk generation
