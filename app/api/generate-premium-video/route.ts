    ...
    try {
        const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [{
                    role: "system",
                    content: `You are a cinematic video director. The user has an image of a PROTAGONIST (product or person) in a scene.
Describe the FULL video scene: natural camera motion, environmental storytelling, atmospheric effects (lighting, particles, smoke, water), and subtle interactions around the protagonist.
The PROTAGONIST must remain recognizable — do not alter its core shape, text, or identity.
You can add environmental dynamics, moving backgrounds, floating particles, light rays, steam, or liquid motion.
Max 40 words, English only.`
                }, {
                    role: "user",
                    content: userContext
                }]
        ...
        return `${userContext}, dynamic cinematic scene, atmospheric lighting, environmental particles, camera orbiting protagonist, 8k ultra-realistic commercial`;
    }
}
