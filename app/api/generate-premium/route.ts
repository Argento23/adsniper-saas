...
async function enhancePromptForStudio(userScene: string, brandName?: string, hasCustomImage?: boolean): Promise<string> {
...
- If children, people or models are mentioned, describe their genuine expressions, posture, and hands holding/interacting with the branded product/logo/item naturally
- Max 60 words
...
"Producto en cocina moderna" → "The premium product elegantly placed on a white quartz countertop in a modern luxury kitchen, warm ambient lighting, subtle reflections, professional commercial product photography, 8k resolution"`
                    ...
                    role: "user",
                    content: `Brand: ${brandName || 'AdSíntesis'}. Scene desired: ${userScene}.${hasCustomImage ? ' Include the uploaded brand product/logo integrated in the scene.' : ''}`
                }]
            })
        });
...
export async function POST(req: Request) {
    try {
        ...
        const enhancedPrompt = await enhancePromptForStudio(
            ...
            brand?.name,
            Boolean(image_base64 && image_base64.length > 50)
        );
        ...
        // 3. Composite brand logo badge & custom image text overlay
