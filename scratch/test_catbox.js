...
async function testCatboxUpload() {
    ...
    formData.append('fileToUpload', blob, 'hex_logo.png');

    console.log("1. Uploading to Catbox...");
    ...
    console.log("✅ Catbox Direct Public Logo URL:", publicUrl);

    // Test URL fetch
    const checkRes = await fetch(publicUrl);
    console.log("Check URL status:", checkRes.status, checkRes.headers.get('content-type'));

    // Call FLUX Redux on Replicate
    console.log("🚀 Requesting FLUX Redux 3D Scene Synthesis with Catbox URL...");
